...
复跑命令、完整数据与实测环境见根 README 的
[压测基线](../README.md#压测基线)，脚本是 `scripts/bench_api.py`。
下面只说结论与当初改了什么。

**这一节早先贴过一张「1/4/32/128 并发 → QPS 240/299/275/257」的表，
但仓库里没有产出它的脚本，数字复跑不出来，已删除。**
现在的表是 `bench_api.py` 跑出来的，任何人都能重跑核对。
...
调整后**降级路径按设计生效**：2026-08-10 在 Python 3.12 上用
`scripts/bench_api.py` 实测，MySQL 路径 64 并发 128/128 全成功、P50 163ms；
128 并发时池被打满，超出的请求在 **5.09 秒**拿到 503 + `Retry-After`，
与 `pool_timeout=5` 精确吻合，其余 216 个正常返回。完整数据见根 README。

**但这套参数当初只配给了 MySQL，SQLite 分支被直接 return 跳过了。**
文件型 SQLite 在 SQLAlchemy 里仍然走 QueuePool，跳过参数不是"没有池"，
而是落到默认池（`size=5, overflow=10, timeout=30`）。后果：
不带 `DATABASE_URL` 启动（也就是上面「本地运行」教的命令）时，
56 并发实测 P50 **30 秒**、QPS 3.7 —— 正是这次改动想消灭的"慢性死亡"，
只是换了个后端又长了回来。现在容量与超时对所有后端一视同仁，
`tests/test_db_pool_config.py` 守着这件事。
...
第二次**完全不发网络请求**，因此不产生 token 费用——
`test_cache_hit_skips_network` 断言的就是 HTTP 调用次数，改坏了会红。
（这里原先写着"3041ms 降到 30ms"，但没有产出该数字的脚本，已改为陈述可测的事实。）

缓存有一个坑值得记：写缓存发生在拿到 HTTP 200 时，那时**还没解析**。
所以模型返回解析不了的内容时，这份坏结果会被钉住一个 TTL，
期间同一对（简历, 岗位）每次命中缓存、每次失败。
`match_resume_to_job` 在解析失败时会调 `ratelimit.cache_delete` 把它删掉。
...
`/matches/refresh` 会对几十个候选岗位逐个调模型。原先它在请求线程里跑完再返回：
用户几十秒白屏、网关一超时这次调用的钱照付、按钮还能重复点。现在改为入队后立刻
返回任务 ID，进度由 `GET /matches/refresh/{task_id}` 轮询。

```bash
arq services.tasks.WorkerSettings      # 在 backend/ 下启动 worker
```

**只异步化了这一个接口。** 爬虫已经有 systemd timer，再塞进队列只是把同一件事
换个地方执行，白多一个要运维的进程——队列要解决的是"用户在等"，不是"任务要定时"。

三个设计取舍：

| 取舍 | 选择 | 理由 |
| --- | --- | --- |
| 队列不可用时 | 退回同步执行（`mode=sync`） | 与限流一致：异步是体验优化，Redis 挂了不能让用户连匹配都跑不了 |
| 失败重试 | `max_tries=1`，不自动重试 | 每次都真金白银调模型，自动重跑等于自动重复付费 |
| 配额扣减时机 | 入队时扣，不等执行 | 否则一个人可以瞬间塞满队列，等 worker 跑起来才发现超额，钱已花掉一部分 |

进度不用 arq 自带的任务状态（它只有"排队/执行中/完成"），另写一个 Redis 哈希
记 `done/total`，因为用户想看的是"评到第几个岗位"。状态里带 `user_id` 并在接口
里核对归属——`task_id` 不校验归属就等于凭 ID 能读别人的匹配结果；归属不符与
任务不存在都返回 404，否则这个接口就成了探测他人任务 ID 的工具。

**worker 必须和 API 加载同一份环境变量。** 实测踩过：worker 没加载 `.env`，
连到了默认的 SQLite 而 API 在 MySQL 上，进程不报错、任务也"成功"，
只是每次返回 0 个匹配。连错库是静默失败，所以启动时会把实际连到的库
打进日志（密码掩码）。
