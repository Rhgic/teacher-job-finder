...
Page({
  ...
  async loadProfile() {
    ...
    this.setData({
      ...
      onboardingNextText: nextItem.done ? "查看岗位订阅" : nextItem.label,
  ...
  openResumeCenter() {
    wx.navigateTo({ url: "/pages/resumes/index" })
  },
  openRecommend() {
  ...
  openOnboardingNext() {
    if (this.data.onboardingNextUrl) {
      if (this.data.onboardingNextUrl === "/pages/rag-ask/index") {
        wx.switchTab({ url: this.data.onboardingNextUrl })
        return
      }
  ...
  showUploadTip() {
    wx.showModal({
      title: "简历上传",
      content: "请到简历中心上传 PDF/TXT 简历，系统会保存并提取基础摘要参与匹配。Word 文件当前只做保存，自动解析有限。",
      showCancel: false
    })
  }
})
