...
ok = await check("岗位接口 /jobs", async () => {
  const res = await fetch(`${apiBase}/jobs?page_size=100`)
  if (!res.ok) throw new Error(`HTTP ${res.status}`)
  const data = await res.json()
  const jobs = normalizeJobList(data)
  if (!jobs) throw new Error("响应不是岗位数组或分页对象")
  if (jobs.length > 0 && !("source_url" in jobs[0])) {
...
ok = await check("已截止岗位投递防线", async () => {
  const jobsRes = await fetch(`${apiBase}/jobs?page_size=100`)
  if (!jobsRes.ok) throw new Error(`/jobs HTTP ${jobsRes.status}`)
  const jobs = normalizeJobList(await jobsRes.json())
  if (!jobs) throw new Error("岗位响应不是岗位数组或分页对象")
  const today = new Date().toISOString().slice(0, 10)
  const expired = jobs.find((job) => {
    const deadline = job.deadline || job.deadline_at
    return deadline && String(deadline).slice(0, 10) < today
  })
  if (!expired) {
