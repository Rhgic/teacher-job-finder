...
function normalizeRemoteRule(item) {
  return {
    ...
    bianzhi: item.need_establishment ? "优先编制" : "不限",
...
function normalizeRemoteResume(item) {
  return {
    id: item.id,
    name: item.file_name,
    completeness: item.structured_content ? 82 : 45,
    updated_at: item.uploaded_at ? String(item.uploaded_at).slice(0, 10) : "已上传",
    highlights: Object.keys(item.structured_content || {}).slice(0, 3)
  }
}
...
function profileToDraft(profile = {}) {
  ...
  return {
    ...
    major: profile.major || "",
    districts: (profile.target_districts || intent.districts || []).join("、") || "全深圳",
    stages: (profile.target_stages || intent.stages || []).join("、") || "小学、初中、高中",
    subjects: (profile.target_subjects || intent.subjects || []).join("、") || profile.subject || "",
    certificate: profile.cert_no || "",
    years: intent.years || "",
    schoolType: intent.school_type || "公办、民办均可"
  }
}
