...
- 每次消息处理返回一份默认折叠的 Agent 执行轨迹：展示语言/意图、风险、演示工具命中、知识库引用数和草稿生成器；轨迹只含服务端固定的安全摘要，不回显买家原文、订单号、工具参数或密钥。
...
2. 展开「Agent 执行轨迹（演示）」：说明语言/意图识别、风险规则、订单与物流工具、知识库检索、草稿生成器的真实处理顺序；轨迹不展示用户原文或工具参数。
3. 展示订单上下文、带知识库引用的低风险草稿，点击“采纳为待发送记录”；强调不会外发。
4. 输入 `I want a refund for order #CB-20512`，展示轨迹最后的「已创建人工审核单」和自动创建的退款审核单。
5. 仍用 agent 打开审核页，说明没有审批按钮；退出后用 `admin / demo1234` 登录，填写理由并通过或驳回。
